...
class ActiveRecord
{
     ...
     * @param string $where
     */
    public function whereExists($where, $boolean = 'exists')
    {
    ...
    {
        if (count($data) < 2) {
            throw new \Exception('sql where error');
        }
    ...
    protected function setWhere($field, $mode = '=', $value = null, $boolean = 'AND')
    {
        ...
        $twhere = $this->db()->getWhere();
        $where = !empty($twhere) ? (' ' . $boolean . ' ' . $where) : $where;
